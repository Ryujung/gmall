...
import com.atguigu.gmall.vo.user.UmsAdminLoginParam;
import com.atguigu.gmall.vo.user.UmsAdminParam;
...
public class UmsAdminController {
    ...
    public Object register(@Valid @RequestBody UmsAdminParam umsAdminParam,BindingResult result) {
        ...
        log.debug("需要注册的用户信息:{}, 校验错误数:{}", umsAdminParam,result.getErrorCount());
        return new CommonResult().success(admin);
    }

    /**
     * 登录成功后,用户信息以jwt令牌的方式返回给前端
     * <p>
     * 关于RequestBody
     * ①如果前端发来json字符串,使用该注解封装
     * ②如果前端发来k-v键值对,不要用该注解
     *
    ...
    public Object getAdminInfo(HttpServletRequest request) {
        String oldToken = request.getHeader(tokenHeader);
        //token加了一个Bearer前缀(配置文件中),在获得oldToken时要截取前缀tokenHead
        String userName = jwtTokenUtil.getUserNameFromToken(oldToken.substring(tokenHead.length()));

        //getOne是Mybatis-plus生成的,且带泛型,和Dubbo不兼容
        //Dubbo无法直接调用mp中带泛型的service
        //实战经验: 如果遇到有泛型的方法,不能使用rpc,需要自己写
        //自动生成的可能有问题
